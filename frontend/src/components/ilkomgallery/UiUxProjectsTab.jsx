...
const UiUxProjectsTab = () => {
  ...
  useEffect(() => {
    projectsService.getAll({ category: 'uiux' })
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <p className="text-theme-muted text-sm"><span className="font-semibold text-accent">{filteredProjects.length}</span> project UI/UX</p>
      </div>
      ...
      {filteredProjects.length > 0 ? (
        ...
        <div className="text-center py-16 bg-theme-secondary rounded-2xl border border-theme">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-accent/10 rounded-full mb-4"><Palette size={40} className="text-accent" /></div>
